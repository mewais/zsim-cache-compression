...
static void PostInitStats(bool perProcessDir, Config& config) {
    ...
    string outputName = config.get<const char*>("sim.outputName", "zsim");

    // Absolute paths for stats files. Note these must be in the global heap.
    const char* pStatsFile = gm_strdup((pathStr + outputName +".h5").c_str());
    const char* evStatsFile = gm_strdup((pathStr + outputName +"-ev.h5").c_str());
    const char* cmpStatsFile = gm_strdup((pathStr + outputName +"-cmp.h5").c_str());
    const char* statsFile = gm_strdup((pathStr + outputName +".out").c_str());
